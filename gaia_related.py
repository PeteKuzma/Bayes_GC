...
class gaia:
	...
	def get_gaia_data(self,cluster,rad):
		'''
	...
	def gaia_u_clean(self,cluster,has_space="no"):
		'''
		...
		if has_space=="no":
			...
			center = coordinates.SkyCoord.from_name(clust)
		elif has_space=="yes":
			center = coordinates.SkyCoord.from_name(cluster)
		RA=center.ra.value
		DEC=center.dec.value
		...
		for i in PB.progressbar(range(len(gr))):
			...
			gr[i,5]=pmr*np.cos(racl)-f_data['pmdec'][i]*np.sin(de)*np.cos(racl)
	...
	def catalog_selection(self,file,rin,rout,pmra_up,pmra_down,pmdec_up,pmdec_down):
		'''
	...
	def iso_from_CMD(self,cluster,survey,magu,magl,magturn,mstep=0.01,bstep=0.01,bsize=0.1,brgb_size=0.3,plot="yes",coll=0.0,colh=1.1,colstep=0.002,cmd_split=0.2):
		...
		if survey=="PS1":
			...
			#	plt.plot(ht['i_R0'],sp3(ht['i_R0']),'g.')
				...
				plt.show()
			xs=np.linspace(magl,magu,10000)
		a1=mag_iso[mag_iso>=(magturn+0.1)]
		print(a1)
		a2=mag_iso[mag_iso<magturn]
		print(a2)
	...
	def weight_calculation_vari(self,cluster,survey,magu,magl,colu,coll,t_rad,turnoff_tip=0,turnoff_base=0,pert=0.4,sig_sel=3):
		f_in=fits.open("{0}/{0}_Gaia_cleaned.fits".format(cluster))
		f_data=Table(f_in[1].data)
		fopen=open("{0}/{0}_isochrone.pickle".format(cluster),'rb')
		turnoff=np.arange(turnoff_tip,turnoff_base+0.001,0.001)
		...
		if survey=="Gaia":
			...
			PC,PC1=sop.curve_fit(self.exp_func,self.mag_range,self.weight,bounds=([-np.inf,-np.inf,-np.inf],[np.inf,0,np.inf]))
			f_data.write("{0}/{0}_FULL_catalog.fits".format(cluster),format="fits",overwrite=True)
...
# 			f_ms=f_data1[f_data1['g_0']>=turnoff_base]
# 			f_rgb=f_data1[f_data1['g_0']<turnoff_tip]
# 			hst,bns=np.histogram(f_ms['w_iso'],bins=np.arange(-30,30,0.01))
# 			mid_bns=bns[:-1]+0.005			
# 			g = models.Gaussian1D()
# 			fit_t = fitting.LevMarLSQFitter()
# 			t1 = fit_t(g, mid_bns, hst)
# 			plt.hist(f_ms['w_iso'],bins=np.arange(-30,30,0.01))
# 			plt.plot(mid_bns,t1(mid_bns))
# 			plt.xlim((-10,10))
# 			plt.show()
# 			hst,bns=np.histogram(f_rgb['w_iso'],bins=np.arange(-30,30,0.01))
# 			mid_bns=bns[:-1]+0.005			
# 			g = models.Gaussian1D()
# 			fit_t = fitting.LevMarLSQFitter()
# 			t2 = fit_t(g, mid_bns, hst)
