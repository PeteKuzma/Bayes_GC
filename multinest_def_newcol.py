...
class PyNM:
	def __init__(self,cluster,radius,version="1",outbase_add=None):
		...
		self.Parameters=["x_pm,CL","y_pm,CL","x_pm,MW","y_pm,MW","x_dsp,MW","y_dsp,MW","f_cl","theta","k","theta2","k2"]
	...
	def loglike_newPM(self,cube, ndim, nparams):
		x_cl,y_cl,x_g,y_g,sx_g,sy_g,fcl,the,c,the2,k=\
	...
	def loglike_mem(self,x_ps,y_ps,x_pm,y_pm,cv_pmraer,cv_pmdecer,cv_coeff,w_par,sample):
		...
		Calculates the membership probability for an individual star
		'''
		gcct=self.L_sat_quad(x_ps,y_ps,sample[:,9],sample[:,10])
