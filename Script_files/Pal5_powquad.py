from multinest_def_NC_power_gam import PyNM as PyNM
print("Loaded succesfully")
PMN=PyNM("Pal5",35,outbase_add="noRC_pow_exp")
import numpy as np
import scipy.stats
...
def prior(cube, ndim, nparams):
	...
	cube[0] = scipy.stats.norm(-2.736,0.064).ppf(cube[0])
	cube[1] = scipy.stats.norm(-2.646,0.064).ppf(cube[1])
	#cube[0] = cube[0]*20-10
#	cube[1] = cube[1]*20-10
	...
	cube[9] = 200*cube[9]-100
	cube[10] = 200*cube[10]-100
	#cube[9] = 2*np.pi*cube[9]-np.pi
	#cube[10] = 10**(12*cube[10]-10)
	#cube[12] = 10**(4*cube[12] - 3)
	cube[11] = np.pi*cube[11]/2.
	#cube[12] = 10**(6*cube[12]-5)
	#cube[12] = 10**(8*cube[12] -4)
	#cube[12] = scipy.stats.norm(0.0381,0.0030).ppf(cube[12])
	#cube[15] = scipy.stats.norm(0.20074,0.032).ppf(cube[15])
	#cube[13] = 10**(3*cube[2] - 3)
	cube[12] = cube[12]*10
	#cube[14] = cube[14]*10
	cube[13] = 10**(8*cube[13]-4)



PMN.PyMultinest_setup(prior,0,50000,0.0381,0.20074,select=False,existing=True,Fadd="_FULL",rmax=4,live_points=200,pm_sel="gnom")
#PMN.PyMultinest_run()
