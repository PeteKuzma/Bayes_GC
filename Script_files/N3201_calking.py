...
PMN=PyNM("NGC3201",35,outbase_add="calking")
import numpy as np
import scipy.stats
...
def prior(cube, ndim, nparams):
	...
	cube[0] = scipy.stats.norm(8,324,0.044).ppf(cube[0])
	cube[1] = scipy.stats.norm(-1.991,0.044).ppf(cube[1])
	...
	#cube[14] = scipy.stats.norm(0.0204,0.0011).ppf(cube[14])
	#cube[15] = scipy.stats.norm(0.414876,0.029).ppf(cube[15])


PMN.PyMultinest_setup(prior,0,50000,0.0204,0.414876,select=False,pm_sel="gnom",existing=True,Fadd="_FULL",rmax=4,live_points=200)
PMN.PyMultinest_run()
