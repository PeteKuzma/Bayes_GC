...
PMN=PyNM("NGC5694",35,outbase_add="calking_gaia")
import numpy as np
import scipy.stats
...
def prior(cube, ndim, nparams):
	...
	cube[0] = scipy.stats.norm(-0.486,0.069).ppf(cube[0])
	cube[1] = scipy.stats.norm(-1.071,0.068).ppf(cube[1])
	...
	#cube[14] = scipy.stats.norm(0.0013,.00005).ppf(cube[14])
	#cube[15] = scipy.stats.norm(0.0999,0.0023).ppf(cube[15])


PMN.PyMultinest_setup(prior,0,50000,0.0013,0.0999,select=False,pm_sel="gnom",existing=True,Fadd="_FULL",rmax=4,live_points=200)
PMN.PyMultinest_run()
