...
PMN=PyNM("NGC6981",35,outbase_add="calking")
import numpy as np
import scipy.stats
...
def prior(cube, ndim, nparams):
	...
	cube[0] = scipy.stats.norm(-1.233,0.062).ppf(cube[0])
	cube[1] = scipy.stats.norm(-3.290,0.061).ppf(cube[1])
	...
	#cube[14] = scipy.stats.norm(0.0076,0.0004).ppf(cube[14])
	#cube[15] = scipy.stats.norm(0.134,0.006).ppf(cube[15])


PMN.PyMultinest_setup(prior,0,50000,0.0076,0.134,select=False,pm_sel="gnom",existing=True,Fadd="_FULL",rmax=4,live_points=200)
PMN.PyMultinest_run()
