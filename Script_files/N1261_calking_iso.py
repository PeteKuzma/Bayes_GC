...
PMN=PyNM("NGC1261",35,outbase_add="calking")
import numpy as np
import scipy.stats
...
def prior(cube, ndim, nparams):
	#cube[0] = GaussianPrior(cube[0],1.632,0.057)  # Prior for x_pm,cl is between -10 to 10.
	#cube[1] = GaussianPrior(cube[1],-2.038,0.057)  # Prior for y_pm,cl is between -10 to 10.
	cube[0] = scipy.stats.norm(1.632,0.057).ppf(cube[0])
	cube[1] = scipy.stats.norm(-2.038,0.057).ppf(cube[1])
	#def prior(cube, ndim, nparams):
	...
	#cube[14] = scipy.stats.norm(0.0059,0.0012).ppf(cube[14]) 
	#cube[15] = scipy.stats.norm(0.0926,0.006).ppf(cube[15])

PMN.PyMultinest_setup(prior,0,50000,0.0059,0.0926,select=False,existing=True,Fadd="_FULL",rmax=4,pm_sel="gnom")
PMN.PyMultinest_run()
