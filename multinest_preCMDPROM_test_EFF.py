...
#PyNM.__init__(self,cluster,radius,prior,inner_radii,sample_size,cr,tr,select=True,pm_sel="norm",live_points=400,existing=False,rmax=4.,Fadd=None,preking=False,outbase_add=None)
        ...
        self.Plummer=self.L_sat_spat_PL(self.x_ps,self.y_ps,self.cr,0,self.rmax)
        self.Parameters=["x_pm,cl","y_pm,cl","x_dsp,cl","y_dsp,cl","x_pm,MW","y_pm,MW","x_dsp,MW","y_dsp,MW","f_cl","f_ev","theta","k","theta2","k2","gamma","xpm_const","ypm_const","gamma_CL"]
    ...
    def PyMultinest_plots_corner(self,setup="complete",save_fig=True):
        ...
        if rank==0:
            try:
                ...
                "$k_{MW}$","$\\theta_{ex}$","$k_{ex}$","$\gamma$","$\Delta\mu_{\\xi,cl}$","$\Delta\mu_{\eta,cl}$","$\gamma_{cl}$"]
    ...
    def loglike_ndisp(self,cube, ndim, nparams):
        x_cl,y_cl,sx_cl,sy_cl,x_g,y_g,sx_g,sy_g,fcl,fev,the,c,the2,k,gam,pmxc,pmyc,gcl=\
        cube[0],cube[1],cube[2],cube[3],cube[4],cube[5],cube[6],cube[7],cube[8],cube[9],cube[10],cube[11],cube[12],cube[13],cube[14],cube[15],cube[16],cube[17]
        mc=(self.PCMD_CL*(\
        ...
        #self.King+(1-fev)*fcl*\
        self.L_sat_spat_PLg(self.x_ps,self.y_ps,self.cr,0,self.rmax,gcl)+(1-fev)*fcl*\
    ...
    def loglike_mem(self,x_ps,y_ps,x_pm,y_pm,cv_pmraer,cv_pmdecer,cv_coeff,w_par,sample,dist,prcl,prmw):
        ...
        gcsp=self.L_sat_spat_PLg(x_ps,y_ps,self.cr,0,self.rmax,sample[:,17])
