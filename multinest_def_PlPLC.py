...
class PyNM:
	def __init__(self,cluster,radius,version="1"):
		...
		self.Parameters=["x_pm,cl","y_pm,cl","x_dsp,cl","y_dsp,cl","x_pm,MW","y_pm,MW","x_dsp,MW","y_dsp,MW","r_h","f_cl","f_rt","power"]
	...
	def PyMultinest_run(self):
		...
		try:
			tstart=time.time()
			pymultinest.run(self.loglike_PlPL, self.Prior, self.N_params,outputfiles_basename=self.outbase_name, \
	...
	def PyMultinest_results(self,setup="complete"):
		try:
			result = pymultinest.solve(LogLikelihood=self.loglike_PlPL, Prior=self.Prior, 
...
#		return mc
	...
	def loglike_PlPL(self,cube, ndim, nparams):
		x_cl,y_cl,sx_cl,sy_cl,x_g,y_g,sx_g,sy_g,rc,fcl,fev,power=\
		...
		self.L_sat_spat_PL(self.x_ps,self.y_ps,rc,self.rmin,self.rmax)+fcl*(1-fev)*self.L_sat_spat_pow(self.x_ps,self.y_ps,rc,self.rmin,self.rmax,power))+\
	...
	def loglike_mem_V2(self,x_ps,y_ps,x_pm,y_pm,cv_pmraer,cv_pmdecer,cv_coeff,w_par,sample):
		...
		'''
		gcsp=self.L_sat_spat_IS(x_ps,y_ps,sample[:,8],self.rmin,self.rmax)
