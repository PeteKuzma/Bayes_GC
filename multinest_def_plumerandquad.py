...
class PyNM:
	def __init__(self,cluster,radius,version="1",outbase_add=None):
		...
		self.Parameters=["x_pm,cl","y_pm,cl","x_disp,cl","y_disp_cl","x_pm,MW","y_pm,MW","x_dsp,MW","y_dsp,MW","f_cl","f_ev","theta","k","theta2","k2","width"]
...
#		z=3*r*r*(1+b*np.cos(np.arctan2(yt_g,xt_g)-the))/(2*np.pi*self.rmax*self.rmax*self.rmax)
#		return z
...
#		return mc
	...
	def L_sat_quad(self,xt_g,yt_g,the,b):
		r=np.sqrt(xt_g**2+yt_g**2)
		mc=r*r*3*(2+b+b*np.cos(2*(np.arctan2(yt_g,xt_g)-the)))/(self.rmax*self.rmax*self.rmax*np.pi*(2+b)*2)
		return mc


#	def L_sat_quad(self,xt_g,yt_g,the,b):
#		r=np.sqrt(xt_g**2+yt_g**2)
#		mc=3*r*(2+b*r+b*r*np.cos(2*(np.arctan2(yt_g,xt_g)-the)))/(2*np.pi*self.rmax*self.rmax*(3+b*self.rmax))
#		return mc	


	def L_sat_quad_x(self,xt_g,yt_g,the,b):
		r=np.sqrt(xt_g**2+yt_g**2)
	...
	def L_spat_stripe(self,sig,rot,k,x_ps,y_ps):
		'''
		...
		y_psL Position of y.
		'''
		z= 1/(sig * np.sqrt(2 * np.pi)) * np.exp( -(((1+k*np.cos(np.arctan2(y_ps,x_ps)-rot)*np.cos(np.arctan2(y_ps,x_ps)-rot)))**2 / (2 * sig**2) ))
		return z
	...
	def loglike_ndisp(self,cube, ndim, nparams):
		x_cl,y_cl,sx_cl,sy_cl,x_g,y_g,sx_g,sy_g,fcl,fev,the,c,the2,ki,sig=\
		...
		self.King+(1-fev)*fcl*\
		self.L_spat_stripe(sig,the2,ki,self.x_ps,self.y_ps)*self.L_pm_GC(x_cl,y_cl,self.x_pm,self.y_pm,self.cv_pmraer,self.cv_pmdecer,self.cv_coeff)\
	...
	def loglike_mem_V3(self,x_ps,y_ps,x_pm,y_pm,cv_pmraer,cv_pmdecer,cv_coeff,w_par,sample):
		...
		'''
		gcsp=self.L_plum_quad(x_ps,y_ps,1,sample[:,8],sample[:,7],self.cr)
		mysp=self.L_sat_grad(x_ps,y_ps,sample[:,5],1,sample[:,6])
		gcpm=self.L_pm_GC(-3.234,-6.719\
	...
	def loglike_mem_v2(self,x_ps,y_ps,x_pm,y_pm,cv_pmraer,cv_pmdecer,cv_coeff,w_par,sample):
		...
		#gcsp=self.Kingi
		gcsp=where(sqrt(x_ps**2+y_ps**2)<=self.tr,self.L_sat_king(x_ps,y_ps,self.cr,self.tr),-1e40)
	...
	def Membership_after_PyNM(self,sample_size,rad,gnom=True):
		...
		try:
			...
			for j in PB.progressbar(range(len(w_par))):
				zvf[j,0],zvf[j,1]=self.loglike_mem_V3(x_ps[j],y_ps[j],x_pm[j],y_pm[j],\
