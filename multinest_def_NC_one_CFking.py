...
class PyNM:
	def __init__(self,cluster,radius,version="1",outbase_add=None):
		...
		self.Parameters=["x_pm,cl","y_pm,cl","x_dsp,cl","y_dsp,cl","x_pm,MW","y_pm,MW","x_dsp,MW","y_dsp,MW","f_cl","theta","k","rc","c"]
	...
	def L_pm_GC_p(self,x_g,y_g,x_pm,y_pm,cv_pmraer,cv_pmdecer,cv_coeff):
		'''
	...
	def L_sat_king_x2(self,r,ah,c):
		mc=r *(( 1/(1+c**2))+(1./(1+((r**2)/(ah**2))))-((2*ah)/(sqrt(ah**2+r**2)*sqrt(1+c**2))))/\
		(pi*(((self.rmax**2)/(1+c**2))+(4*(ah-sqrt(ah**2+self.rmax**2))*ah)/(sqrt(1+c**2))\
		+log(1+((self.rmax**2)/(ah**2)))*ah**2))
		return mc
	...
	def L_pm_MW_o(self,x_g,y_g,sx_g,sy_g,x_pm,y_pm,cv_pmraer,cv_pmdecer,cv_coeff):
		'''
	...
	def loglike_ndisp(self,cube, ndim, nparams):
		x_cl,y_cl,sx_cl,sy_cl,x_g,y_g,sx_g,sy_g,fcl,the,c,rc,rt=\
		...
		where(self.r<=(rt),self.L_sat_king(self.x_ps,self.y_ps,rc,rt),0)\
