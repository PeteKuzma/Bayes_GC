...
class PyNM:
	def __init__(self,cluster,radius,version="1",outbase_add=None):
		...
		"x_dsp,MW","y_dsp,MW","f_cl","f_ev","theta","k","theta2","k2"]
	...
	def loglike_ndisp(self,cube, ndim, nparams):
		x_cl,y_cl,x_g,y_g,sx_g,sy_g,fcl,fev,the,c,the2,k=\
	...
	def loglike_mem(self,x_ps,y_ps,x_pm,y_pm,cv_pmraer,cv_pmdecer,cv_coeff,w_par,sample):
		...
		Calculates the membership probability for an individual star
		'''
		gcct=self.L_sat_quad(x_ps,y_ps,sample[:,10],sample[:,11])
	...
	def Membership_after_PyNM(self,sample_size,rad):
		...
		try:
			...
			tot_sample=np.concatenate((s1,s2,s3,s4,s5,s6,s7,s8,s9,s10,s11,s12),axis=1)
