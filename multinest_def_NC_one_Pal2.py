...
class PyNM:
	def __init__(self,cluster,radius,version="1",outbase_add=None):
		...
		"x_dsp,MW","y_dsp,MW","f_cl","f_ev","theta","k","theta2","k2","rc","x_dsp,ts","y_dsp,ts"]
	...
	def loglike_ndisp(self,cube, ndim, nparams):
		x_cl,y_cl,sx_cl,sy_cl,x_g,y_g,sx_g,sy_g,fcl,fev,the,c,the2,k,rc,sx_ts,sy_ts=\
	...
	def loglike_mem(self,x_ps,y_ps,x_pm,y_pm,cv_pmraer,cv_pmdecer,cv_coeff,w_par,sample):
		...
		tspm=self.L_pm_MW(sample[:,0],sample[:,1],sample[:,15],sample[:,16]\
		,x_pm,y_pm,cv_pmraer,cv_pmdecer,cv_coeff)
		fcl=sample[:,8]
		fev=sample[:,9]
	...
	def Membership_after_PyNM(self,sample_size,rad):
		...
		try:
			...
			s17=np.random.choice(ST[16],sample_size)[np.newaxis].T
			tot_sample=np.concatenate((s1,s2,s3,s4,s5,s6,s7,s8,s9,s10,s11,s12,s13,s14,s15,s16,s17),axis=1)
			...
			f_data.write("{0}_mem_list_tot_{1}.fits".format(self.cluster,self.outbase_add),format="fits",overwrite=True)			#f_d3.write("{0}_mem_list_0_3.fits".format(self.cluster),format="fits",overwrite=True)
