...
class PyNM:
	def __init__(self,cluster,radius,version="1",outbase_add=None):
		...
		self.Parameters=["x_pm,cl","y_pm,cl","x_pm,MW","y_pm,MW","f_cl","f_ev","theta","k","theta2","k2","rc"]
	...
	def loglike_ndisp(self,cube, ndim, nparams):
		x_cl,y_cl,x_g,y_g,fcl,fev,the,c,the2,k,rc=\
	...
	def loglike_mem(self,x_ps,y_ps,x_pm,y_pm,cv_pmraer,cv_pmdecer,cv_coeff,w_par,sample):
		...
		gcsp=self.L_sat_spat_PLt(x_ps,y_ps,sample[:,10],self.rmin,self.rmax)
	...
	def Membership_after_PyNM(self,sample_size,rad):
		...
		try:
			...
			tot_sample=np.concatenate((s1,s2,s3,s4,s5,s6,s7,s8,s9,s10,s11,s12,s13),axis=1)
