...
class PyNM:
	def __init__(self,cluster,radius,version="1",outbase_add=None):
		...
		self.Parameters=["x_pm,cl","y_pm,cl","x_pm,MW","y_pm,MW","x_dsp,MW","y_dsp,MW","f_cl","theta","k","theta2","k2","rc","g"]
	...
	def L_pm_GC0_old(self,x_g,y_g,x_pm,y_pm,cv_pmraer,cv_pmdecer,cv_coeff):
		'''
		Max log-likelihood from the proper motions of the Satellite. The variables are:
		x_g = cube for proper motion in x-profection
		y_g = cube for proper motion in y-profection
	...
	def L_sat_king_x(self,r,ah,c):
		mc=where(r<=(c*ah),r *( 1/(1+c**2)+1./(1+(r**2)/(ah**2))-2*ah/(sqrt(ah**2+r**2)*sqrt(1+c**2)))/\
		(pi*(self.rmax**2/(1+c**2)+4*(ah-sqrt(ah**2+self.rmax**2))*ah/(sqrt(1+c**2))\
		+log(1+self.rmax**2/ah**2)*ah**2)),-1.e100)
		return mc
	...
	def L_plum_quad(self,xt_g,yt_g,s,k,the,c):
		r=np.sqrt(xt_g*xt_g+yt_g*yt_g)
		theta=np.arctan2(yt_g,xt_g)
		nom=(c*c+self.rmax*self.rmax)*(2*c*c*k*r*r+k*r*r*r*r+c*c*c*c*(k+2*s*r)+k*(c*c+r*r)*(c*c+r*r)*np.cos(2*(theta-the)))
		dom=(2*self.rmax*np.pi*(k*self.rmax*self.rmax+c*c*(k+self.rmax*s)))*(c*c+k*k)(c*c+k*k)
		g=nom/dom
		return g
	...
	def L_pm_MW(self,x_g,y_g,sx_g,sy_g,x_pm,y_pm,cv_pmraer,cv_pmdecer,cv_coeff):
		...
		expd=2*cv_pmraer**2*((-1+cv_coeff**2)*cv_pmdecer**2-sy_g**2)-2*sx_g**2*(cv_pmdecer**2+sy_g**2)
		mc=(1/(2*pi*sqrt(cv_pmdecer**2*cv_pmraer**2-cv_coeff**2*cv_pmdecer**2*cv_pmraer**2\
		+cv_pmdecer**2*sx_g**2+sy_g**2*cv_pmraer**2)))*exp(expn/expd)
		return mc

	def L_pm_MW_old(self,x_g,y_g,sx_g,sy_g,x_pm,y_pm,cv_pmraer,cv_pmdecer,cv_coeff):
		'''
	...
	def L_power_grad(self,xt_g,yt_g,ah,g,k,T,y):
		t=np.arctan2(yt_g,xt_g)
		r=np.sqrt(xt_g**2+yt_g**2)
		nom=r*(1+((r*(1+k*np.cos(t-T)*np.cos(t-T)))/ah)**2)**-y
		dem=quad(lambda x: self.theta(self.rmax,x,k,T,ah,y),0,2*np.pi)[0]
		return nom/dem

	def theta(self,r,t,k,T,c,y):
		nom=(c**2)*(1-(1+((r**2*(1+k*np.cos(t-T))**4)/(c**2)))**(1-y))
		dem=2*(-1+y)*(1+k*np.cos(t-T))**4
		return nom/dem

	def loglike_ndisp(self,cube, ndim, nparams):
		x_cl,y_cl,sx_cl,sy_cl,x_g,y_g,sx_g,sy_g,fcl,the,c,the2,k=\
		cube[0],cube[1],cube[2],cube[3],cube[4],cube[5],cube[6],cube[7],cube[8],cube[9],cube[10],cube[11],cube[12]
		mc=(np.log(self.L_pm_GC(x_cl,y_cl,self.x_pm,self.y_pm,self.cv_pmraer,self.cv_pmdecer,self.cv_coeff)*fcl*\
		self.L_power_grad(self.x_ps,self.y_ps,rc,self.rmax,k,the2,g)
	...
	def loglike_ndisp2(self,cube, ndim, nparams):
		x_cl,y_cl,sx_cl,sy_cl,x_g,y_g,sx_g,sy_g,fcl,fev,the,c,the2,rc=\
		...
		self.L_sat_spat_PLt(self.x_ps,self.y_ps,rc,self.rmin,self.rmax)+(1-fev)*fcl*\
	...
	def loglike_mem(self,x_ps,y_ps,x_pm,y_pm,cv_pmraer,cv_pmdecer,cv_coeff,w_par,sample):
		...
		#gcsp=where(sqrt(x_ps**2+y_ps**2)<=sample[:,15],self.L_sat_king(x_ps,y_ps,sample[:,14],sample[:,15]),0)	
		gcsp=self.L_power_grad(x_ps,y_ps,sample[:,11],self.rmax,sample[:,10],sample[:,9],sample[:,12])
		#gcpm=self.L_pm_MW(sample[:,0],sample[:,1],sample[:,],sample[:,3]\
		,x_pm,y_pm,cv_pmraer,cv_pmdecer,cv_coeff)
		mwpm=self.L_pm_MW(sample[:,2],sample[:,3],sample[:,4]\
		,sample[:,5],x_pm,y_pm,cv_pmraer,cv_pmdecer,cv_coeff)
		mwsp=self.L_sat_grad(x_ps,y_ps,sample[:,7],1,sample[:,8])
		pcpm=self.L_pm_GC(sample[:,0],sample[:,1],\
		x_pm,y_pm,cv_pmraer,cv_pmdecer,cv_coeff)
		fcl=sample[:,6]
		#fev=sample[:,9]
		mc_cl=(fcl*gcsp*gcpm)/\
		(fcl*gcsp*gcpm+(1-fcl)*mwpm*mwsp)
		mc_co=(fcl*gcsp*gcpm)/\
		(fcl*gcsp*gcpm+(1-fcl)*mwpm*mwsp)
		mc_ts=(fcl*gcsp*gcpm)/\
		(fcl*gcsp*gcpm+(1-fcl)*mwpm*mwsp)
	...
	def Membership_after_PyNM_V2(self,sample_size,rad):
		...
		try:
			...
			tot_sample=np.concatenate((s1,s2,s3,s4,s5,s6,s7,s8,s9,s10,s11,s12,s13,s14,s15,s16),axis=1)
			zvf=zeros((len(f_data),6))
			print("Begin to calculate Membership probability.")
