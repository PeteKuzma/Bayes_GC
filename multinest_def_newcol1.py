...
class PyNM:
	...
	def PyMultinest_setup(self,prior,inner_radii,sample_size,pmra,pmdec,select=True,pm_sel="norm",live_points=400,existing=False,rmax=4.,Fadd=None):
		self.Prior=prior
	...
	def PyMultinest_run(self):
		...
		try:
			tstart=time.time()
			pymultinest.run(self.loglike_newPM, self.Prior, self.N_params,outputfiles_basename=self.outbase_name, \
	...
	def PyMultinest_results(self,setup="complete"):
		try:
			result = pymultinest.solve(LogLikelihood=self.loglike_newPM, Prior=self.Prior, 
	...
	def loglike_ndisp(self,cube, ndim, nparams):
		x_cl,y_cl,sx_cl,sy_cl,x_g,y_g,sx_g,sy_g,fcl,fev,the,c,the2,k,rc=\
	...
	def loglike_newPM(self,cube, ndim, nparams):
		x_g,y_g,sx_g,sy_g,fcl,the,c,the2,k=\
		cube[0],cube[1],cube[2],cube[3],cube[4],cube[5],cube[6],cube[7],cube[8]
		mc=(np.log(fcl*
		self.L_sat_quad(self.x_ps,self.y_ps,the2,k)*self.L_pm_GC(self.pmra,self.pmdec,self.x_pm,self.y_pm,self.cv_pmraer,self.cv_pmdecer,self.cv_coeff)\
		+self.L_sat_grad(self.x_ps,self.y_ps,the,1,c)*\
		(1-fcl)*self.L_pm_MW(x_g,y_g,sx_g,sy_g,self.x_pm,self.y_pm,self.cv_pmraer,self.cv_pmdecer,self.cv_coeff)\
		)).sum()
		return mc	
	...
	def loglike_mem(self,x_ps,y_ps,x_pm,y_pm,cv_pmraer,cv_pmdecer,cv_coeff,w_par,sample):
		...
		#gcsp=self.L_sat_spat_PLt(x_ps,y_ps,sample[:,14],self.rmin,self.rmax)
		gcpm=self.L_pm_GC(self.pmra,self.pmdec\
		,x_pm,y_pm,cv_pmraer,cv_pmdecer,cv_coeff)
		mwpm=self.L_pm_MW(sample[:,0],sample[:,1],sample[:,2]\
		,sample[:,3],x_pm,y_pm,cv_pmraer,cv_pmdecer,cv_coeff)
		...
		#fev=sample[:,9]
		mc_cl=((fcl*gcct*gcpm)/\
		(fcl*gcct*gcpm+(1-fcl)*mwpm*mwsp))
		return np.nanmean(mc_cl),np.nanstd(mc_cl)
