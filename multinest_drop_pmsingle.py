...
#PyNM.__init__(self,cluster,radius,prior,inner_radii,sample_size,cr,tr,select=True,pm_sel="norm",live_points=400,existing=False,rmax=4.,Fadd=None,preking=False,outbase_add=None)
    ...
    def loglike_ndisp(self,cube, ndim, nparams):
        ...
        mc=self.PCMD_CL*\
        self.L_pm_TS(x_cl,y_cl,sx_cl,sy_cl,gamx,gamy,self.x_ps,self.y_ps,self.x_pm,self.y_pm,self.cv_pmraer,self.cv_pmdecer,self.cv_coeff)*(fev*fcl*\
        self.King+(1-fev)*fcl*\
    ...
    def loglike_mem(self,x_ps,y_ps,x_pm,y_pm,cv_pmraer,cv_pmdecer,cv_coeff,w_par,sample,dist,prcl,prmw):
        ...
        fcl=sample[:,8]
        fev=sample[:,9]
        mc_cl=(gccmd*((fcl*fev)*gcsp*tspm+(fcl*(1-fev)*tspm*gcct))/\
        (((gccmd*(fcl*fev*gcsp*tspm+(fcl*(1-fev)*tspm*gcct)))+(1-fcl*fev-fcl*(1-fev))*mwpm*mwsp*mwcmd)))
        mc_co=(gccmd*fcl*fev*gcsp*tspm)/\
        (((gccmd*(fcl*fev*gcsp*tspm+(fcl*(1-fev)*tspm*gcct)))+(1-fcl*fev-fcl*(1-fev))*mwpm*mwsp*mwcmd))
