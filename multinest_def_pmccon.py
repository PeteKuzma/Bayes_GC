...
class PyNM:
	def __init__(self,cluster,radius,version="1",outbase_add=None):
		...
		self.Parameters=["x_dsp,cl","y_dsp,cl","x_pm,MW","y_pm,MW","x_dsp,MW","y_dsp,MW","f_cl","f_ev","theta","k"]
...
#		return mc
	...
	def loglike_str(self,cube, ndim, nparams):
		sx_cl,sy_cl,x_g,y_g,sx_g,sy_g,fcl,fev,the,c=\
		cube[0],cube[1],cube[2],cube[3],cube[4],cube[5],cube[6],cube[7],cube[8],cube[9]
		mc=(np.log((self.L_pm_MW(-3.1925,-6.7445,sx_cl,sy_cl,self.x_pm,self.y_pm,self.cv_pmraer,self.cv_pmdecer,self.cv_coeff)*fev*(1-fcl)*\
		self.L_sat_spat_PL(self.x_ps,self.y_ps,0.1,self.rmin,self.rmax)+fev*fcl*\
		self.L_sat_constt(self.x_ps,self.y_ps)*self.L_pm_MW(-3.1925,-6.7445,sx_cl,sy_cl,self.x_pm,self.y_pm,self.cv_pmraer,self.cv_pmdecer,self.cv_coeff)\
		+self.L_sat_grad(self.x_ps,self.y_ps,the,1,c)*\
		(1-fcl*fev - fev*(1-fcl))*self.L_pm_MW(x_g,y_g,sx_g,sy_g,self.x_pm,self.y_pm,self.cv_pmraer,self.cv_pmdecer,self.cv_coeff)\
		))).sum()
		return mc
	...
	def loglike_mem(self,x_ps,y_ps,x_pm,y_pm,cv_pmraer,cv_pmdecer,cv_coeff,w_par,sample):
		...
		Calculates the membership probability for an individual star
		'''
		gcct=self.L_sat_constt(x_ps,y_ps)
		gcsp=self.L_sat_spat_PL(x_ps,y_ps,0.1,self.rmin,self.rmax)
		gcpm=self.L_pm_MW(-3.1925,-6.7445,sample[:,0],sample[:,1]\
		,x_pm,y_pm,cv_pmraer,cv_pmdecer,cv_coeff)
		mwpm=self.L_pm_MW(sample[:,2],sample[:,3],sample[:,4]\
		,sample[:,5],x_pm,y_pm,cv_pmraer,cv_pmdecer,cv_coeff)
		mwsp=self.L_sat_grad(x_ps,y_ps,sample[:,8],1,sample[:,9])
		fcl=sample[:,6]
		fts=sample[:,7]
		mc_cl=((fcl*fts)*gcsp*gcpm+(fcl*(1-fts)*gcpm*gcct))/\
		(fcl*fts*gcsp*gcpm+(fcl*(1-fts)*gcpm*gcct)+(1-fcl*fts-fcl*(1-fts))*mwpm*mwsp)
