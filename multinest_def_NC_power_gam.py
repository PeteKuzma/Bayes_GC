...
class PyNM:
	def __init__(self,cluster,radius,version="1",outbase_add=None):
		...
		self.Parameters=["x_pm,cl","y_pm,cl","x_dsp,cl","y_dsp,cl","x_pm,MW","y_pm,MW","x_dsp,MW","y_dsp,MW","f_cl","a","b","theta2","gamma","k"]
	...
	def PyMultinest_setup(self,prior,inner_radii,sample_size,cr,tr,select=True,pm_sel="norm",live_points=400,existing=False,rmax=4.,Fadd=None,preking=False):
		...
		self.cr=cr
		self.dist=M2['dist']
		self.theta=np.arctan2(M2['dec_g'],M2['ra_g'])
	...
	def PyMultinest_run(self):
		...
		try:
			tstart=time.time()
			pymultinest.run(self.loglike_V2, self.Prior, self.N_params,outputfiles_basename=self.outbase_name, \
	...
	def PyMultinest_results(self,setup="complete"):
		try:
			result = pymultinest.solve(LogLikelihood=self.loglike_V2, Prior=self.Prior, 
	...
	def L_sat_power_quad(self,xt_g,yt_g,r,theta,k,the,ga,rc):
		nom=(-1+ga)*r*((1+r*r/(rc*rc))**(-ga))*(2+k+k*np.cos(2*(theta-the)))*((1+self.rmax*self.rmax/(rc*rc))**ga)
		dem=np.pi*(2+k)*(-self.rmax*self.rmax+rc*rc*(-1+(1+self.rmax*self.rmax/(rc*rc))**ga))
		mc=nom/dem
		return mc



		#nom=2*(-1+be)*(-1+ga)*(r*((1+r*r/(rc*rc))**-be)*np.cos(theta-the)*np.cos(theta-the)+r*((1+r*r/(rc*rc))**-ga)*np.sin(theta-the)*np.sin(theta-the))
		#dem=np.pi*((rc*rc+self.rmax*self.rmax)*(-((1+self.rmax*self.rmax/(rc*rc))**-ga)*(-1+be)-((1+self.rmax*self.rmax/(rc*rc))**-be)*(-1+ga))+rc*rc*(-2 +be+ga))
		#mc = nom/dem
		#return
	...
	def L_sat_gradr(self,xt_g,yt_g,the,a):
		r=np.sqrt(xt_g*xt_g+yt_g*yt_g)
		theta=np.arctan2(yt_g,xt_g)
		mc = r*(1+r*a*np.cos(theta-the))/(np.pi*self.rmax**2)
		return mc

	def L_sat_expgrad(self,xt_g,yt_g,a,b):
		r=np.sqrt(xt_g*xt_g+yt_g*yt_g)
		the=np.arctan2(yt_g,xt_g)
		nom=np.exp(a*r*np.cos(the)+b*r*np.sin(the))*r*(a*np.cos(the)+b*np.sin(the))*(a*np.cos(the)+b*np.sin(the))
		dem=2*(np.pi+np.exp(a*self.rmax*np.cos(the)+b*self.rmax*np.sin(the))*np.pi*(-1+a*self.rmax*np.cos(the)+b*self.rmax*np.sin(the)))
		mc=nom/dem
		return mc
	...
	def L_sat_quad_x(self,xt_g,yt_g,the):
		r=np.sqrt(xt_g**2+yt_g**2)
		mc=r*np.cos(np.arctan2(yt_g,xt_g)-the)*np.cos(np.arctan2(yt_g,xt_g)-the)/(self.rmax**2*np.pi)
		return mc
	...
	def loglike_V2(self,cube, ndim, nparams):
		x_cl,y_cl,sx_cl,sy_cl,x_g,y_g,sx_g,sy_g,fmw,a,b,the,ga,k=\
		...
		self.L_sat_power_quad(self.x_ps,self.y_ps,self.dist,self.theta,k,the,ga,self.cr)+\
			 fmw*self.L_pm_MW(x_g,y_g,sx_g,sy_g,self.x_pm,self.y_pm,self.cv_pmraer,self.cv_pmdecer,self.cv_coeff)\
			*self.L_sat_expgrad(self.x_ps,self.y_ps,a,b))).sum()
		return mc
	...
	def loglike_mem_v2(self,x_ps,y_ps,x_pm,y_pm,cv_pmraer,cv_pmdecer,cv_coeff,w_par,sample):
		...
		mwpm=self.L_pm_MW(sample[:,4],sample[:,5],sample[:,6]\
		,sample[:,7],x_pm,y_pm,cv_pmraer,cv_pmdecer,cv_coeff)
		mwsp=self.L_sat_gradr(x_ps,y_ps,sample[:,10],sample[:,11])
