...
class PyNM:
	def __init__(self,cluster,radius,version="1",outbase_add=None):
		...
		self.Parameters=["x_pm,cl","y_pm,cl","x_dsp,cl","y_dsp,cl","x_pm,MW","y_pm,MW","x_dsp,MW","y_dsp,MW","f_cl","theta","k","rc","rt"]
		self.N_params = len(self.Parameters)
		self.cluster_F=cluster

	def PyMultinest_setup(self,prior,inner_radii,sample_size,select=True,pm_sel="norm",live_points=400,existing=False,rmax=4.,Fadd=None,preking=False):
		self.Prior=prior
...
#		self.tr=tr
	...
	def loglike_ndisp(self,cube, ndim, nparams):
		...
		where(sqrt(self.x_ps**2+self.y_ps**2)<=rt,self.L_sat_king(self.x_ps,self.y_ps,rc,rt),0)
