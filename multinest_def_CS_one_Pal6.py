...
class PyNM:
	def __init__(self,cluster,radius,version="1",outbase_add=None):
		...
		self.Parameters=["x_pm,cl","y_pm,cl","x_pm,MW","y_pm,MW",\
		"f_cl","f_ev","theta","k","theta2","k2"]
	...
	def loglike_ndisp(self,cube, ndim, nparams):
		x_cl,y_cl,x_g,y_g,fcl,fev,the,c,the2,k=\
		cube[0],cube[1],cube[2],cube[3],cube[4],cube[5],cube[6],cube[7],cube[8],cube[9]
		mc=(np.log(self.L_pm_GC(x_cl,y_cl,self.x_pm,self.y_pm,self.cv_pmraer,self.cv_pmdecer,self.cv_coeff)*fev*fcl*\
		...
		(1-fcl)*self.L_pm_GC(x_g,y_g,self.x_pm,self.y_pm,self.cv_pmraer,self.cv_pmdecer,self.cv_coeff)\
		)).sum()
		return mc
	...
	def loglike_mem(self,x_ps,y_ps,x_pm,y_pm,cv_pmraer,cv_pmdecer,cv_coeff,w_par,sample):
		...
		Calculates the membership probability for an individual star
		'''
		gcct=self.L_sat_quad(x_ps,y_ps,sample[:,8],sample[:,9])
		gcsp=self.L_sat_spat_PLt(x_ps,y_ps,self.rad_sel/60.,self.rmin,self.rmax)
		gcpm=self.L_pm_GC(sample[:,0],sample[:,1]\
		,x_pm,y_pm,cv_pmraer,cv_pmdecer,cv_coeff)
		mwpm=self.L_pm_GC(sample[:,2],sample[:,3]\
		,x_pm,y_pm,cv_pmraer,cv_pmdecer,cv_coeff)
		mwsp=self.L_sat_grad(x_ps,y_ps,sample[:,6],1,sample[:,7])
		...
		(fcl*fev*gcsp*gcpm+fcl*(1-fev)*tspm*gcct)
