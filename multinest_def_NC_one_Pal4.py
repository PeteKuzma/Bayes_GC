...
class PyNM:
	def __init__(self,cluster,radius,version="1",outbase_add=None):
		...
		self.Parameters=["x_pm,cl","y_pm,cl","x_pm,MW","y_pm,MW","x_dsp,MW","y_dsp,MW","f_cl","f_ev","theta","k","theta2","k2","rc"]
		self.N_params = len(self.Parameters)
		self.cluster_F=cluster

	def PyMultinest_setup(self,prior,inner_radii,sample_size,select=True,pm_sel="norm",live_points=100,existing=False,rmax=4.,Fadd=None):
		self.Prior=prior
	...
	def loglike_ndisp(self,cube, ndim, nparams):
		x_cl,y_cl,x_g,y_g,sx_g,sy_g,fcl,fev,the,c,the2,k,rc=\
	...
	def loglike_mem(self,x_ps,y_ps,x_pm,y_pm,cv_pmraer,cv_pmdecer,cv_coeff,w_par,sample):
		...
		gcsp=self.L_sat_spat_PLt(x_ps,y_ps,sample[:,12],self.rmin,self.rmax)
