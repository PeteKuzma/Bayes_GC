...
#PyNM.__init__(self,cluster,radius,prior,inner_radii,sample_size,cr,tr,select=True,pm_sel="norm",live_points=400,existing=False,rmax=4.,Fadd=None,preking=False,outbase_add=None)
        self.Parameters=["x_pm,cl","y_pm,cl","x_dsp,cl","y_dsp,cl","x_pm,MW","y_pm,MW","x_dsp,MW","y_dsp,MW","f_cl+ex","f_cl","theta","k","theta2","k2","gamma"]
    ...
    def loglike_ndisp(self,cube, ndim, nparams):
        ...
        where(self.dist>=(self.factor*self.TR),self.L_sat_quad_r_mni0(self.x_ps,self.y_ps,the2,gam,k,self.TR),0)*\
    ...
    def loglike_mem(self,x_ps,y_ps,x_pm,y_pm,cv_pmraer,cv_pmdecer,cv_coeff,w_par,sample,dist,prcl,prmw,cr,tr):
        ...
        gcct=where(dist<=tr*self.factor,self.L_sat_quad_r_mni0(x_ps,y_ps,sample[:,12],sample[:,14],sample[:,13],tr),0)
        ...
        gcsp=where(dist<=tr,self.L_sat_king(x_ps,y_ps,cr,tr),1e-99)
