...
class PyNM:
	...
	def PyMultinest_setup(self,prior,inner_radii,sample_size,select=True,pm_sel="norm",live_points=400,existing=False,rmax=4.,Fadd=None,step=20):
		self.Prior=prior
		...
		self.w_par=M2['w_iso']
		ang=np.arange(-np.pi,np.pi + np.pi/step,np.pi/step)
		self.N=np.zeros(len(ang)-1)
		self.ang=np.zeros(len(ang)-1)
		for i in range(len(self.N)):
			self.N[i]=len(M2[(np.arctan2(M2['dec_g'],M2['ra_g'])>=ang[i])&(np.arctan2(M2['dec_g'],M2['ra_g'])<ang[i+1])])
			self.ang[i]=(ang[i]+ang[i+1])/2.
		self.R=np.sqrt(self.x_ps**2+self.y_ps**2)
		self.theta=np.arctan2(self.y_ps,self.x_ps)
		self.theta2=np.zeros((len(self.theta)))
		self.N_sec=np.zeros(len(self.x_ps))
		for i in range(len(self.x_ps)):
			if self.theta[i]>=0:
				self.theta2[i]=self.theta[i]
			else:
				self.theta2[i]=2*np.pi+self.theta[i]
		for i in range(len(self.x_ps)):
			C1=self.theta[(self.theta>=(self.theta[i]-np.pi/step))]
			C1=len(C1[C1<=(self.theta[i]+np.pi/step)])
			C2=self.theta2[(self.theta2>=(self.theta2[i]-np.pi/step))]
			C2=len(C2[C2<=(self.theta2[i]+np.pi/step)])
			if C1==C2:
				self.N_sec[i]=C1
			elif C1 > C2:
				self.N_sec[i]=C1
	...
	def PyMultinest_run(self):
		...
		try:
			tstart=time.time()
			pymultinest.run(self.loglike_PlC, self.Prior, self.N_params,outputfiles_basename=self.outbase_name, \
	...
	def PyMultinest_results(self,setup="complete"):
		try:
			result = pymultinest.solve(LogLikelihood=self.loglike_PlC, Prior=self.Prior, 
	...
	def L_sat_grad(self,xt_g,yt_g,the,a,b):
		z=(a+b*np.cos(np.arctan2(yt_g,xt_g)-the))/(np.pi*a*2*self.rmax)
		#z1=rmax-rmin + 0.5*(rmax**2 - rmin**2)*np.cos(np.arctan2(yt_g,xt_g)-the)
		return z
		#n1=a*np.sqrt(xt_g**2+yt_g**2)*0.5+(1/3.)*b*np.sqrt(xt_g**2+yt_g**2)*np.sqrt(xt_g**2+yt_g**2)*np.cos(np.arctan2(yt_g,xt_g)-the)
		#n2=0.5*(rmax**2)*a+(1/3.)*b*(rmax**3*np.cos(np.arctan2(yt_g,xt_g)-the))
		#m=(n1/n2)
		#return m
	...
	def L_spat_gradg(self,sig_g,e,k,theta,N,ang):
		'''
		sig_g = estimating the spread of the cluster distribution 
		fromt the w-parameter.
		w_par
		'''
		likelihood = (1./(sqrt(2*pi*sig_g**2)))*exp(-(((e+k*np.cos(ang-theta))-N)**2)/(2.*sig_g**2.))
		return likelihood
...
#		return mc
	...
	def loglike_PlC(self,cube, ndim, nparams):
		x_cl,y_cl,sx_cl,sy_cl,x_g,y_g,sx_g,sy_g,fcl,rot,k=\
		cube[0],cube[1],cube[2],cube[3],cube[4],cube[5],cube[6],cube[7],cube[8],cube[9],cube[10]
		mc=(np.log(self.L_pm_MW(x_cl,y_cl,sx_cl,sy_cl,self.x_pm,self.y_pm,self.cv_pmraer,self.cv_pmdecer,self.cv_coeff)*(1-fcl)\
		*self.L_sat_spat_PL(self.x_ps,self.y_ps,0.1,self.rmin,self.rmax)+\
		(fcl*self.L_pm_MW(x_g,y_g,sx_g,sy_g,self.x_pm,self.y_pm,self.cv_pmraer,self.cv_pmdecer,self.cv_coeff)\
		*self.L_sat_grad(self.x_ps,self.y_ps,rot,1,k)))).sum()
		return mc

#	def loglike_str(self,cube, ndim, nparams):
#		x_cl,y_cl,sx_cl,sy_cl,x_g,y_g,sx_g,sy_g,ah,c,fcl,rot=\
#		cube[0],cube[1],cube[2],cube[3],cube[4],cube[5],cube[6],cube[7],cube[8],cube[9],cube[10],cube[11]
#		mc=((self.L_pm_MW(x_cl,y_cl,sx_cl,sy_cl,self.x_pm,self.y_pm,self.cv_pmraer,self.cv_pmdecer,self.cv_coeff)*(1-fcl)*\
#		self.L_spat_plumC(self.x_ps,self.y_ps,ah,c)+\
#		fcl*self.L_pm_MW(x_g,y_g,sx_g,sy_g,self.x_pm,self.y_pm,self.cv_pmraer,self.cv_pmdecer,self.cv_coeff)\
#		*self.L_sat_grad(self.x_ps,self.y_ps,rot,self.rmax,self.rmin))).sum()
#		return mc

	def loglike_str(self,cube, ndim, nparams):
		x_cl,y_cl,sx_cl,sy_cl,x_g,y_g,sx_g,sy_g,fcl=\
		cube[0],cube[1],cube[2],cube[3],cube[4],cube[5],cube[6],cube[7],cube[8]
		mc=(np.log(self.L_pm_MW(x_cl,y_cl,sx_cl,sy_cl,self.x_pm,self.y_pm,self.cv_pmraer,self.cv_pmdecer,self.cv_coeff)*(1-fcl)*\
		+\
		fcl*self.L_pm_MW(x_g,y_g,sx_g,sy_g,self.x_pm,self.y_pm,self.cv_pmraer,self.cv_pmdecer,self.cv_coeff))).sum()
		return mc

	def loglike_str1(self,cube, ndim, nparams):
		x_cl,y_cl,sx_cl,sy_cl,x_g,y_g,sx_g,sy_g,fcl,rot,ah,a,k=\
		cube[0],cube[1],cube[2],cube[3],cube[4],cube[5],cube[6],cube[7],cube[8],cube[9],cube[10],cube[11],cube[12]
		mc=((self.L_pm_MW(x_cl,y_cl,sx_cl,sy_cl,self.x_pm,self.y_pm,self.cv_pmraer,self.cv_pmdecer,self.cv_coeff)*(1-fcl)*\
		self.L_sat_spat_PL(self.x_ps,self.y_ps,ah,self.rmin,self.rmax)+\
		fcl*self.L_pm_MW(x_g,y_g,sx_g,sy_g,self.x_pm,self.y_pm,self.cv_pmraer,self.cv_pmdecer,self.cv_coeff)\
		*self.L_sat_grad(self.x_ps,self.y_ps,rot,self.rmax,self.rmin,a,k))).sum()
		return mc


	def loglike(self,cube, ndim, nparams):
		x_cl,y_cl,sx_cl,sy_cl,x_g,y_g,sx_g,sy_g,rc,fcl=\
		cube[0],cube[1],cube[2],cube[3],cube[4],cube[5],cube[6],cube[7],cube[8],cube[9]
		mc=(log(self.L_pm_MW(x_cl,y_cl,sx_cl,sy_cl,self.x_pm,self.y_pm,self.cv_pmraer,self.cv_pmdecer,self.cv_coeff)*((fcl*fev)*\
		self.L_sat_spat_PL(self.x_ps,self.y_ps,rc,self.rmin,self.rmax)+fcl*(1-fev))+\
		(1-fev*fcl-fcl*(1-fev))*((self.L_pm_MW(x_g,y_g,sx_g,sy_g,self.x_pm,self.y_pm,self.cv_pmraer,self.cv_pmdecer,self.cv_coeff)\
		)))).sum()#*self.L_sat_const(self.x_ps,self.y_ps))))).sum()
		return mc

	def loglike_pow(self,cube, ndim, nparams):
		x_cl,y_cl,sx_cl,sy_cl,x_g,y_g,sx_g,sy_g,rc,fcl,power=\
		cube[0],cube[1],cube[2],cube[3],cube[4],cube[5],cube[6],cube[7],cube[8],cube[9],cube[10]
		mc=(log(self.L_pm_MW(x_cl,y_cl,sx_cl,sy_cl,self.x_pm,self.y_pm,self.cv_pmraer,self.cv_pmdecer,self.cv_coeff)*(1-fcl)*\
		self.L_sat_spat_pow(self.x_ps,self.y_ps,rc,self.rmin,self.rmax,power)\
		+fcl*(self.L_pm_MW(x_g,y_g,sx_g,sy_g,self.x_pm,self.y_pm,self.cv_pmraer,self.cv_pmdecer,self.cv_coeff)\
	...
	def loglike_2(self,cube, ndim, nparams):
		x_cl,y_cl,sx_cl,sy_cl,x_g,y_g,sx_g,sy_g,rc,fcl,fev,tailx,taily=\
		cube[0],cube[1],cube[2],cube[3],cube[4],cube[5],cube[6],cube[7],cube[8],cube[9],cube[10],cube[11],cube[12]
		mc=(log(self.L_pm_MW(x_cl,y_cl,sx_cl,sy_cl,self.x_pm,self.y_pm,self.cv_pmraer,self.cv_pmdecer,self.cv_coeff)*(fcl*fev)*\
		self.L_sat_spat_PL(self.x_ps,self.y_ps,rc,self.rmin,self.rmax)+fcl*(1-fev)*\
		self.L_spat_MVG(tailx,taily,self.x_ps,self.y_pm,self.cv_raer,self.cv_deer,self.cv_radeccov)*\
		self.L_pm_MW(x_cl,y_cl,sx_cl,sy_cl,self.x_pm,self.y_pm,self.cv_pmraer,self.cv_pmdecer,self.cv_coeff)+\
		(1-fev*fcl-fcl*(1-fev))*self.L_pm_MW(x_g,y_g,sx_g,sy_g,self.x_pm,self.y_pm,self.cv_pmraer,self.cv_pmdecer,self.cv_coeff)\
		*self.L_sat_const(self.x_ps,self.y_ps))).sum()
		return mc
	...
	def loglike_IS(self,cube, ndim, nparams):
		...
		cube[0],cube[1],cube[2],cube[3],cube[4],cube[5],cube[6],cube[7],cube[8],cube[9],cube[10],cube[11]
		mc=(log(self.L_pm_MW(x_cl,y_cl,sx_cl,sy_cl,self.x_pm,self.y_pm,self.cv_pmraer,self.cv_pmdecer,self.cv_coeff)*((fcl*fev)*\
		self.L_sat_spat_PL(self.x_ps,self.y_ps,rc,self.rmin,self.rmax)+fcl*(1-fev)*self.L_sat_spat_IS(self.x_ps,self.y_ps,ah,0,self.rmax))+\
			((1-fev*fcl-fcl*(1-fev))*self.L_pm_MW(x_g,y_g,sx_g,sy_g,self.x_pm,self.y_pm,self.cv_pmraer,self.cv_pmdecer,self.cv_coeff)\
			*self.L_sat_const(self.x_ps,self.y_ps)))).sum()
		return mc
	...
	def loglike_mem_V2(self,x_ps,y_ps,x_pm,y_pm,cv_pmraer,cv_pmdecer,cv_coeff,w_par,sample):
		...
		'''
		gcsp=self.L_sat_spat_PL(x_ps,y_ps,0.1,self.rmin,self.rmax)
		mysp=self.L_sat_grad(x_ps,y_ps,sample[:,9],1,sample[:,10])
