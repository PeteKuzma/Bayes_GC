...
#PyNM.__init__(self,cluster,radius,prior,inner_radii,sample_size,cr,tr,select=True,pm_sel="norm",live_points=400,existing=False,rmax=4.,Fadd=None,preking=False,outbase_add=None)
        self.Parameters=["x_pm,cl","y_pm,cl","x_dsp,cl","y_dsp,cl","x_pm,MW","y_pm,MW","x_dsp,MW","y_dsp,MW","f_cl+ex","f_cl","theta","k","theta2","k2","gamma","rt"]
    ...
    def loglike_ndisp(self,cube, ndim, nparams):
        x_cl,y_cl,sx_cl,sy_cl,x_g,y_g,sx_g,sy_g,fcl,fev,the,c,the2,k,gam,rt=\
        ...
        self.L_sat_king(self.x_ps,self.y_ps,self.krc,rt,self.rmin)+(1-fev)*fcl*\
        #self.King+(1-fev)*fcl*\
        #self.Plum+(1-fev)*fcl*\
    ...
    def loglike_mem(self,x_ps,y_ps,x_pm,y_pm,cv_pmraer,cv_pmdecer,cv_coeff,w_par,sample,dist,prcl,prmw,cr,tr):
        ...
        gcsp=self.L_sat_king(x_ps,y_ps,self.krc,sample[:,15],self.rmin)
