...
class PyNM:
	def __init__(self,cluster,radius,version="1",outbase_add=None):
		...
		self.Parameters=["x_pm,cl","y_pm,cl","x_dsp,cl","y_dsp,cl","x_pm,MW","y_pm,MW","x_dsp,MW","y_dsp,MW","f_cl"]
	...
	def PyMultinest_run(self):
		...
		try:
			tstart=time.time()
			pymultinest.run(self.loglike_str, self.Prior, self.N_params,outputfiles_basename=self.outbase_name, \
	...
	def PyMultinest_results(self,setup="complete"):
		try:
			result = pymultinest.solve(LogLikelihood=self.loglike_str, Prior=self.Prior, 
	...
	def L_sat_grad(self,xt_g,yt_g,the,rmax,rmin,a,k):
		z=a*(1+k*np.sqrt(xt_g**2+yt_g**2)*np.cos(np.arctan2(yt_g,xt_g)-the))
...
#		return mc
	...
	def loglike_PlPL(self,cube, ndim, nparams):
		x_cl,y_cl,sx_cl,sy_cl,x_g,y_g,sx_g,sy_g,rc,fcl,fev,ah,power=\
		...
		self.L_sat_spat_PL(self.x_ps,self.y_ps,rc,self.rmin,self.rmax)+fcl*(1-fev)*self.L_sat_spat_pow(self.x_ps,self.y_ps,ah,0,self.rmax,power))+\
...
#		return mc
	...
	def loglike_mem_V2(self,x_ps,y_ps,x_pm,y_pm,cv_pmraer,cv_pmdecer,cv_coeff,w_par,sample):
		...
		fmw=sample[:,8]
		mc=((1-fmw)*gcpm)/((1-fmw)*gcpm+(fmw*mwpm))
	...
	def Membership_after_PyNM_V2(self,sample_size,rad):
		...
		try:
			...
			s8=np.random.choice(ST[7],sample_size)[np.newaxis].T
			s9=np.random.choice(ST[8],sample_size)[np.newaxis].T
			tot_sample=np.concatenate((s1,s2,s3,s4,s5,s6,s7,s8,s9),axis=1)
