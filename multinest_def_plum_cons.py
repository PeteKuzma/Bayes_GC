...
class PyNM:
	def __init__(self,cluster,radius,version="1",outbase_add=None):
		...
		self.Parameters=["x_pm,cl","y_pm,cl","x_dsp,cl","y_dsp,cl","x_pm,MW","y_pm,MW","x_dsp,MW","y_dsp,MW","f_cl","ah"]
		self.N_params = len(self.Parameters)
		self.cluster_F=cluster

	def PyMultinest_setup(self,prior,inner_radii,sample_size,select=True,pm_sel="norm",live_points=400,existing=False,rmax=4.,Fadd=None):
		self.Prior=prior
	...
	def L_sat_grad(self,xt_g,yt_g,the,a):
		z=a+np.sqrt(xt_g**2+yt_g**2)*np.cos(np.arctan2(yt_g,xt_g)-the)
		#z1=rmax-rmin + 0.5*(rmax**2 - rmin**2)*np.cos(np.arctan2(yt_g,xt_g)-the)
		return z
		#n1=np.sqrt(xt_g**2+yt_g**2)+np.sqrt(xt_g**2+yt_g**2)*np.sqrt(xt_g**2+yt_g**2)*np.cos(np.arctan2(yt_g,xt_g)-the)
		#n2=0.5*(rmax**2 -rmin**2)+(1/3.)*(rmax**3*np.cos(np.arctan2(yt_g,xt_g)-the)-rmin**3*np.cos(np.arctan2(yt_g,xt_g)-the))
		#m=(n1/n2)
		#return m
...
#		return mc
	...
	def loglike_PlC(self,cube, ndim, nparams):
		x_cl,y_cl,sx_cl,sy_cl,x_g,y_g,sx_g,sy_g,fcl,ah=\
		cube[0],cube[1],cube[2],cube[3],cube[4],cube[5],cube[6],cube[7],cube[8],cube[9]
...
#		return mc
	...
	def loglike_mem_V2(self,x_ps,y_ps,x_pm,y_pm,cv_pmraer,cv_pmdecer,cv_coeff,w_par,sample):
		...
		'''
		gcsp=self.L_sat_spat_PL(x_ps,y_ps,sample[:,9],self.rmin,self.rmax)
		...
		mw=(fmw*mwpm*mysp)/((1-fmw)*gcpm*gcsp+(fmw*mwpm*mysp))
		cp=(1-fmw)*gcpm*gcsp
		mp=(fmw*mwpm*mysp)
		return np.nanmean(mc),np.nanstd(mc),np.nanmean(mw),np.nanstd(mw),np.nanmean(cp),np.nanmean(mp)
	...
	def Membership_after_PyNM_V2(self,sample_size,rad):
		...
		try:
			...
			tot_sample=np.concatenate((s1,s2,s3,s4,s5,s6,s7,s8,s9,s10),axis=1)
			zvf=zeros((len(f_data),6))
			print("Begin to calculate Membership probability.")
			for j in PB.progressbar(range(len(w_par))):
				zvf[j,0],zvf[j,1],zvf[j,2],zvf[j,3],zvf[j,4],zvf[j,5]=self.loglike_mem_V2(x_ps[j],y_ps[j],x_pm[j],y_pm[j],\
					cv_pmraer[j],cv_pmdecer[j],cv_coeff[j],w_par[j],tot_sample)
			f_data['cl_mean']=zvf[:,0]
			f_data['cl_std']=zvf[:,1]
			f_data['MW_mean']=zvf[:,2]
			f_data['MW_std']=zvf[:,3]
			f_data['cp_mean']=zvf[:,4]
			f_data['mp_mean']=zvf[:,5]
