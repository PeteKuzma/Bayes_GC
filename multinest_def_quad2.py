...
class PyNM:
	def __init__(self,cluster,radius,version="1",outbase_add=None):
		...
		self.Parameters=["x_pm,cl","y_pm,cl","x_dsp,cl","y_dsp,cl","x_pm,MW","y_pm,MW","x_dsp,MW","y_dsp,MW","f_cl","f_ev","theta","k","theta2"]
	...
	def PyMultinest_run(self):
		...
		try:
			tstart=time.time()
			pymultinest.run(self.loglike_ndisp2, self.Prior, self.N_params,outputfiles_basename=self.outbase_name, \
	...
	def PyMultinest_results(self,setup="complete"):
		try:
			result = pymultinest.solve(LogLikelihood=self.loglike_ndisp2, Prior=self.Prior,\
...
#		return mc
	...
	def loglike_ndisp(self,cube, ndim, nparams):
		...
		mc=(np.log(self.L_pm_MW(x_cl,y_cl,sx_cl,sy_cl,self.x_pm,self.y_pm,self.cv_pmraer,self.cv_pmdecer,self.cv_coeff)*fev*fcl*\
		self.L_sat_spat_PLt(self.x_ps,self.y_ps,2.73/60.,self.rmin,self.rmax)+(1-fev)*fcl*\
	...
	def loglike_ndisp2(self,cube, ndim, nparams):
		x_cl,y_cl,sx_cl,sy_cl,x_g,y_g,sx_g,sy_g,fcl,fev,the,c,the2=\
		cube[0],cube[1],cube[2],cube[3],cube[4],cube[5],cube[6],cube[7],cube[8],cube[9],cube[10],cube[11],cube[12]
		mc=(np.log(self.L_pm_MW(x_cl,y_cl,sx_cl,sy_cl,self.x_pm,self.y_pm,self.cv_pmraer,self.cv_pmdecer,self.cv_coeff)*fev*fcl*\
		self.L_sat_spat_PLt(self.x_ps,self.y_ps,2.73/60.,self.rmin,self.rmax)+(1-fev)*fcl*\
		self.L_sat_quad_x(self.x_ps,self.y_ps,the2)*self.L_pm_GC(x_cl,y_cl,self.x_pm,self.y_pm,self.cv_pmraer,self.cv_pmdecer,self.cv_coeff)\
		+self.L_sat_grad(self.x_ps,self.y_ps,the,1,c)*\
		(1-fcl)*self.L_pm_MW(x_g,y_g,sx_g,sy_g,self.x_pm,self.y_pm,self.cv_pmraer,self.cv_pmdecer,self.cv_coeff)\
		)).sum()
		return mc
	...
	def loglike_mem(self,x_ps,y_ps,x_pm,y_pm,cv_pmraer,cv_pmdecer,cv_coeff,w_par,sample):
		...
		gcsp=self.L_sat_spat_PLt(x_ps,y_ps,5/60.,self.rmin,self.rmax)
		gcpm=self.L_pm_MW(sample[:,0],sample[:,1],sample[:,2],sample[:,3]\
		,x_pm,y_pm,cv_pmraer,cv_pmdecer,cv_coeff)
		mwpm=self.L_pm_MW(sample[:,4],sample[:,5],sample[:,6]\
		,sample[:,7],x_pm,y_pm,cv_pmraer,cv_pmdecer,cv_coeff)
		...
		fcl=sample[:,8]
		fev=sample[:,9]
		mc_cl=(((fcl*fev)*gcsp*gcpm+(fcl*(1-fev)*gcpm*gcct))/\
		(fcl*fev*gcsp*gcpm+(fcl*(1-fev)*gcpm*gcct)+(1-fcl*fev-fcl*(1-fev))*mwpm*mwsp))
	...
	def Membership_after_PyNM(self,sample_size,rad):
		...
		try:
			...
			tot_sample=np.concatenate((s1,s2,s3,s4,s5,s6,s7,s8,s9,s10,s11,s12,s13,s14),axis=1)
