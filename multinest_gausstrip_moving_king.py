...
#PyNM.__init__(self,cluster,radius,prior,inner_radii,sample_size,cr,tr,select=True,pm_sel="norm",live_points=400,existing=False,rmax=4.,Fadd=None,preking=False,outbase_add=None)
    ...
    def L_sat_gauss_stripe(self,xt_g,yt_g,rot,k):
        r=sqrt(xt_g**2+yt_g**2)
        z=(1/(sqrt(2*np.pi*k*k)))*exp(-0.5*((r*np.cos(arctan2(yt_g,xt_g)-rot))/k)**2)
        return z
    ...
    def loglike_ndisp(self,cube, ndim, nparams):
        ...
        self.King+(1-fev)*fcl*\
        where(np.sqrt(self.x_ps*self.x_ps+self.y_ps*self.y_ps)>=self.tr,self.L_sat_gauss_stripe(self,xt_g,yt_g,the2,k),0)*\
    ...
    def loglike_mem(self,x_ps,y_ps,x_pm,y_pm,cv_pmraer,cv_pmdecer,cv_coeff,w_par,sample,dist,prcl,prmw):
        ...
        '''
        gcct=where(np.sqrt(x_ps*x_ps+y_ps*y_ps)>=self.tr,self.L_sat_quad_r(x_ps,y_ps,sample[:,12],sample[:,13]),0)
        #gcct=self.L_sat_quad_r(x_ps,y_ps,sample[:,12],0,sample[:,13])
