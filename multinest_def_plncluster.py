...
class PyNM:
	def __init__(self,cluster,radius,version="1",outbase_add=None):
		...
		self.Parameters=["x_pm,MW","y_pm,MW","x_dsp,MW","y_dsp,MW","f_cl","theta","k","theta2","k2"]
	...
	def PyMultinest_run(self):
		...
		try:
			tstart=time.time()
			pymultinest.run(self.loglike_ndispnc, self.Prior, self.N_params,outputfiles_basename=self.outbase_name, \
	...
	def PyMultinest_results(self,setup="complete"):
		try:
			result = pymultinest.solve(LogLikelihood=self.loglike_ndispnc, Prior=self.Prior, 
	...
	def L_sat_quad(self,xt_g,yt_g,the,b):
		r=np.sqrt(xt_g**2+yt_g**2)
		mc=r*(2+b+b*np.cos(2*(np.arctan2(yt_g,xt_g)-the)))/(self.rmax**2*np.pi*(2+b))
		return mc
...
#		return mc
	...
	def loglike_str(self,cube, ndim, nparams):
		x_cl,y_cl,sx_cl,sy_cl,x_g,y_g,sx_g,sy_g,fcl,fev,the,c,the2,k,rh=\
		...
		self.L_sat_spat_PLt(self.x_ps,self.y_ps,rh,self.rmin,self.rmax)+(1-fev)*fcl*\
		self.L_sat_quad(self.x_ps,self.y_ps,the2,k)*self.L_pm_MW(x_cl,y_cl,sx_cl,sy_cl,self.x_pm,self.y_pm,self.cv_pmraer,self.cv_pmdecer,self.cv_coeff)\
	...
	def loglike_ndisp(self,cube, ndim, nparams):
		...
		mc=(np.log(self.L_pm_MW(x_cl,y_cl,sx_cl,sy_cl,self.x_pm,self.y_pm,self.cv_pmraer,self.cv_pmdecer,self.cv_coeff)*fev*fcl*\
		self.L_sat_spat_PLt(self.x_ps,self.y_ps,5/60.,self.rmin,self.rmax)+(1-fev)*fcl*\
	...
	def loglike_ndispnc(self,cube, ndim, nparams):
		x_g,y_g,sx_g,sy_g,fcl,the,c,the2,k=\
		cube[0],cube[1],cube[2],cube[3],cube[4],cube[5],cube[6],cube[7],cube[8]
		mc=(np.log(fcl*\
		self.L_sat_quad(self.x_ps,self.y_ps,the2,k)*self.L_pm_GC(-3.1925,-6.7445,self.x_pm,self.y_pm,self.cv_pmraer,self.cv_pmdecer,self.cv_coeff)\
		+self.L_sat_grad(self.x_ps,self.y_ps,the,1,c)*\
		(1-fcl)*self.L_pm_MW(x_g,y_g,sx_g,sy_g,self.x_pm,self.y_pm,self.cv_pmraer,self.cv_pmdecer,self.cv_coeff)\
		)).sum()
		return mc
	...
	def loglike(self,cube, ndim, nparams):
		...
		cube[0],cube[1],cube[2],cube[3],cube[4],cube[5],cube[6],cube[7],cube[8],cube[9]
		mc=((self.L_pm_MW(x_cl,y_cl,sx_cl,sy_cl,self.x_pm,self.y_pm,self.cv_pmraer,self.cv_pmdecer,self.cv_coeff)*((fcl*fev)*\
	...
	def loglike_mem(self,x_ps,y_ps,x_pm,y_pm,cv_pmraer,cv_pmdecer,cv_coeff,w_par,sample):
		...
		Calculates the membership probability for an individual star
		'''
		gcct=self.L_sat_quad(x_ps,y_ps,sample[:,7],sample[:,8])
		#gcsp=self.L_sat_spat_PLt(x_ps,y_ps,2.37/60.,self.rmin,self.rmax)
		gcpm=self.L_pm_GC(-3.1925,-6.7445,x_pm,y_pm,cv_pmraer,cv_pmdecer,cv_coeff)
		mwpm=self.L_pm_MW(sample[:,0],sample[:,1],sample[:,2],sample[:,3]\
		,x_pm,y_pm,cv_pmraer,cv_pmdecer,cv_coeff)
		#mwpm=self.L_pm_MW(sample[:,4],sample[:,5],sample[:,6]\
		#,sample[:,7],x_pm,y_pm,cv_pmraer,cv_pmdecer,cv_coeff)
		mwsp=self.L_sat_grad(x_ps,y_ps,sample[:,5],1,sample[:,6])
		fcl=sample[:,4]
		fev=sample[:,8]
		mc_cl=(fcl*gcpm*gcct)/\
		((fcl*gcpm*gcct)+(1-fcl)*mwpm*mwsp)
		mc_co=fcl+1
		mc_ts=fcl+2
		#mc_co=(fcl*fev*gcsp*gcpm)/\
		#(fcl*fev*gcsp*gcpm+fcl*(1-fev)*gcpm*gcct)
		#mc_ts=(fcl*(1-fev)*gcpm*gcct)/\
		#(fcl*fev*gcsp*gcpm+fcl*(1-fev)*gcpm*gcct)
