...
#PyNM.__init__(self,cluster,radius,prior,inner_radii,sample_size,cr,tr,select=True,pm_sel="norm",live_points=400,existing=False,rmax=4.,Fadd=None,preking=False,outbase_add=None)
        ...
        self.Parameters=["x_pm,cl","y_pm,cl","x_dsp,cl","y_dsp,cl","x_pm,MW","y_pm,MW","x_dsp,MW","y_dsp,MW","f_cl","f_ev","theta","k","theta2","k2","gamma","tr","cr"]
    ...
    def PyMultinest_plots_corner(self,setup="complete",save_fig=True):
        ...
        if rank==0:
            try:
                ...
                "$k_{MW}$","$\\theta_{ex}$","$k_{ex}$","$\gamma$","$r_{t}$","$r_{c}$"]
    ...
    def L_sat_king_orig(self,xt_g,yt_g,ah,rt):
        '''
        USE AFTER EXPERIMENTIN WITH KING WITH CONCENTRATION
        '''
        r=sqrt(xt_g**2+yt_g**2)
        mc=np.where(r<rt,r *( 1/(r*r+ah*ah)+1./(ah*ah+rt*rt)-2/(sqrt(ah*ah+r*r)*sqrt(ah*ah+rt*rt)))/\
        (pi*((self.tr**2+4*(ah-sqrt(ah**2+self.tr**2))*sqrt(ah**2+rt**2))/(ah**2+rt**2)\
        +log(1+self.tr**2/ah**2))),1.0e-99)
        return mc

    def L_sat_king(self,xt_g,yt_g,ah,c):
        r=sqrt(xt_g**2+yt_g**2)
        num=(1+c*c)*r*(1/(1+c*c)-2*ah/(sqrt(1+c*c)*sqrt(ah*ah+r*r))+1/(1+r*r/(ah*ah)))
        dem=ah*pi*(c*c*ah+4*sqrt(1+c*c)*(ah-sqrt((1+c*c)*ah*ah))+(1+c*c)*ah*log(1+c*c))
        mc=np.where(r<(ah*c),num/dem,1.0e-99)
        return mc
    ...
    def loglike_ndisp(self,cube, ndim, nparams):
        x_cl,y_cl,sx_cl,sy_cl,x_g,y_g,sx_g,sy_g,fcl,fev,the,c,the2,k,gam,tr,cr=\
        ...
        self.L_sat_king(self.x_ps,self.y_ps,cr,tr)+(1-fev)*fcl*\
    ...
    def loglike_mem(self,x_ps,y_ps,x_pm,y_pm,cv_pmraer,cv_pmdecer,cv_coeff,w_par,sample,dist):
        ...
        gcsp=self.L_sat_king(x_ps,y_ps,sample[:,16],sample[:,15])
